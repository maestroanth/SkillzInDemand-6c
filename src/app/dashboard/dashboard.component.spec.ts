...
class RouterStub {//so you stub the router and the service to fake the API call see providers below
    navigateByUrl(url: string) { return url; }
    //if the router has a parameter (such as id), you stub that too. It's done in hero-detail.component.spec.ts. the actual stub can be found at testing/router-stubs.ts
}
...
function compileAndCreate() {
  beforeEach( async(() => {
    TestBed.configureTestingModule({
      providers: [
          { provide: HeroService, useClass: FakeHeroService },//so you stub the router and the service to fake the API call
